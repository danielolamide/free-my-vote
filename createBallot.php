<?php
	...
	if ((!isset($_SESSION['userName']))||($_SESSION['uType']==="user")) {
		...
		if(isset($_POST['submit'])){
			$ballot_name = $con->real_escape_string($_POST['ballotName']);
			$number_of_voters = $con->real_escape_string($_POST['voterNumber']);
			...
			$checkBallot = mysqli_query($con,"SELECT * FROM ballot");
			
			if(mysqli_num_rows($checkBallot)<1)
			{	
				$insert1 = $con->query("INSERT INTO ballot(ballot_name,voters) VALUES('$ballot_name','$number_of_voters')");
				$votes =0;
			$insert2 = $con->query("INSERT INTO p_candidates(name,votes) VALUES('$candi1','$votes')");
			$insert3 = $con->query("INSERT INTO p_candidates(name,votes) VALUES('$candi2','$votes')");
			$insert4 = $con->query("INSERT INTO p_candidates(name,votes) VALUES('$candi3','$votes')");
			header("location: createBallot1.php");
		
			}
			else{
				
				$ballotmsg = "You can't have more than one active ballot";		
			}
...
<body>
	...
	<div id="ballot-div">
		<form id="create-ballot-form" method="post" action="createBallot.php">
			...
			<div id="ballot-form-content">
				...
				<?php if ($ballotmsg!=="") {
					echo"<span style ='font-size:20px; color: white;'>";
					echo $ballotmsg ; 
					echo "</span>";
